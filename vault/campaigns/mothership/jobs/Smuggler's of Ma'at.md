---
layout: layout.njk
title: Emergency Extraction, Lower Tombs of Ma'at
pay: 10% profit share (1mcr per artifact)
faction:
  - Smoking Darkness Reclamations
summary: Late-phase salvage support needed on Ma’at. Surface watch and deep-zone coordination under hazardous weather.
location: Ma'at Ruins - Lower Tombs
job_kind:
  - extraction
  - combat
  - smuggling
risk: High
tags:
created: 2025-11-01T16:57
updatedAt: 2025-11-05T19:24
publish: true
theme: mothership
type: job
campaign: mothership
image: /static/img/smuggler.png
---

## Situation Report

Target breached and rendezvous set; extraction of a crew member from lower target required for job completion and payment: coordinates incoming.

## Mission Briefing

### Primary Objective

Independent recovery group seeks discreet personnel to provide on-site security and extraction assistance during a late-stage reclamation operation in the Ma’at badlands. Task involves monitoring a surface launch vehicle, maintaining a low profile near restricted excavation zones, and providing emergency support to a field team working below-ground.

### Operational Overview

- Coordinate with subterranean survey team as needed
- Assist with rapid extraction of personnel and materials
- Observe radio silence unless directly contacted by field lead

**EXTREME HAZARDS CONFIRMED:**
- Severe environmental hazards throughout the operational zone

## Compensation

**Profit Share:** 10% of total artifact sale value

**Estimated Value:** 20kcr per artifact recovered intact.

**Potential Earnings:** Variable based on extraction success and artifact recovery.

### Environmental Factors

Weather conditions deteriorating. Time-sensitive extraction window.

### Equipment Provided

Appropriate recovery and extraction equipment provided.

---

**Contract Status:** URGENT    
**Extraction Window:** Closing fast

[Accept Contract](#) | [Request Additional Intel](#)