...
export default function (eleventyConfig) {
  ...
  // Explicit Nunjucks + Liquid registrations (belt & suspenders)
  ...
  // (Optional) easy access in templates if you need helpers
  ...
  eleventyConfig.amendLibrary("njk", (env) => {
    ...
    // console.log("Nunjucks filters:", Object.keys(env.filters).sort()); // uncomment to debug locally
  });

  /* ---------- Other filters/collections you already use ---------- */
  ...
  eleventyConfig.addFilter("sortBy", (arr, keyPath) =>
    (arr || []).slice().sort((a,b) =>
      String(get(a,keyPath) ?? "").localeCompare(String(get(b,keyPath) ?? ""), undefined, { numeric:true, sensitivity:"base" })
    )
  );
  eleventyConfig.addFilter("concat", (a,b) => ([...(a||[]), ...(b||[])]));
  eleventyConfig.addFilter("collect", (keys, collections) => (keys || []).flatMap(k => collections?.[k] || []));
